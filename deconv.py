...
if __name__ == "__main__":
    ...
    if VERBOSE:
        plt.scatter(l, b, s=2, c=Gapp, cmap='hell', vmin=Gmin,
                    vmax=Gmax+1, edgecolors='none', rasterized=True)
        ...
        if blow <= -bmin*d2r:  # selection region in the southern Galactic hemisphere
            ...
            l1 = lmin(bb)
            l2 = 2 * lsym - l1
            plt.plot(np.hstack((l1, l2[::-1], l1[0])) / d2r,
                     np.hstack((bb, bb[::-1], bb[0])) / d2r, 'g')
    ...
    def modelDensity(params, truedens=False):
        ...
        slope_in = logrho(dens_knots[0], der=1)  # d[log(rho)]/d[log(r)], log-slope at lower radius
        slope_out = logrho(dens_knots[-1], der=1)
        if slope_in <= -3.0 or slope_out >= -3.0:
            raise RuntimeError('Density has invalid asymptotic slope: inner=%.2f, outer=%.2f'
                               % (slope_in, slope_out))
        ...
        norm = agama.integrateNdim(
            ...
            upper=[1, np.sin(bupp),                Gmax+4*DMerr], toler=1e-6
        )[0]
        ...
        if blow <= -bmin*d2r:
            norm += agama.integrateNdim(
                ...
                upper=[1, np.sin(-bmin*d2r), Gmax+4*DMerr], toler=1e-6
            )[0]
    ...
    def likelihood(params):
        ...
        try:
            ...
            sigmar2 = np.exp(2*modelSigma(params_sigmar)(logr_samp))  # squared rad vel dispersion
            ...
            cov_pmbb += pmberr2_samp  # but with actual Gaia data should also use off-diagonal term
    ...
    def plotprofiles(chain, plotname=''):
        ...
        if (plotname == 'converged') or VERBOSE:
            ...
            # retrieve density profiles of each model in chain, compute 16,50,84 percentiles
            ...
            for i in range(len(chain)):
                results_r[i] = np.exp(
                    modelSigma(chain[i, len(knots_logr)-1: 2*len(knots_logr)-1])(lr)
                )
            ...
            sigmar_obs = (np.histogram(
                logr_obs, bins=lrhist, weights=vr_obs**2
            )[0] / count_obs)**0.5
            sigmat_obs = (np.histogram(
                logr_obs, bins=lrhist, weights=vt_obs**2
            )[0] / count_obs)**0.5
            axs[0, 0].plot(np.repeat(rhist, 2)[1:-1], np.repeat(sigmar_obs, 2),
                           'g--', label='with SFs and Errors $\sigma_\mathrm{rad}$', alpha=0.3)
            axs[0, 1].plot(np.repeat(rhist, 2)[1:-1], np.repeat(sigmat_obs, 2),
                           'b--', label='with SFs and Errors $\sigma_\mathrm{tan}$', alpha=0.3)
        ...
        if plotname == 'converged':
            ...
            if true_path is not None:
                profiles = np.stack([r, dens_low, dens_med, dens_upp, truedens,
                                     low_r, med_r, upp_r, truesigr,
                                     low_t, med_t, upp_t, truesigt], axis=1)
                write_csv(
                    profiles,
            ...
            histograms = np.stack([np.repeat(rhist, 2)[1:-1],
                                   np.repeat(rho_obs, 2),
                                   np.repeat(sigmar_obs, 2),
                                   np.repeat(sigmat_obs, 2)], axis=1)
    ...
    write_csv(finals_data, "Menc_beta_final.csv",
