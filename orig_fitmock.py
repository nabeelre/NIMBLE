...
Gmin  = 10.0   # min apparent G magnitude (exclude nearby stars)
...
def makeMock(density, potential, beta0, r_a, nbody):
    ...
    # here use a simple empirical formula estimated from EDR3, but it should be replaced by PyGaia's output
    PMerr = numpy.maximum(1.0 / (21.5-Gapp) / (1 + 0.1*(21.5-Gapp)**2), 0.015)
    pml  += numpy.random.normal(size=nbody) * PMerr
    pmb  += numpy.random.normal(size=nbody) * PMerr
...
l, b, Gapp, pml, pmb, vlos, PMerr, vloserr, true_sigmar, true_sigmat = \
    makeMock(den, pot, beta0=-0.5, r_a=60.0, nbody=30000)
...
if True:
    ...
    x,y,z,vx,vy,vz = agama.getGalactocentricFromGalactic(
        l * d2r, b * d2r, dist_obs, pml * 4.74, pmb * 4.74, vlos)
